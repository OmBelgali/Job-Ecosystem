...
- pps/web - Next.js web app (auth, dashboard, API routes).
- pps/worker - Background worker, queues, and schedulers.
...
- ssets - Brand and font assets.
- output - Generated artifacts (for example PDFs).
- 	mp - Temporary files (for example intermediate PDFs).
...
- Web app: Next.js app in pps/web
- Worker: background worker in pps/worker
